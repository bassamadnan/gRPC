...
openssl req -newkey rsa:4096 -nodes -keyout client-key.pem -out client-req.pem -subj "/CN=localhost" -addext "subjectAltName = DNS:localhost,IP:127.0.0.1"

# 5. Use CA's private key to sign client's CSR and get back the signed certificate
openssl x509 -req -in client-req.pem -days 60 -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -out client-cert.pem -extfile <(printf "subjectAltName=DNS:localhost,IP:127.0.0.1")
